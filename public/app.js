...
async function init(){
  ...
  if(!localStorage.getItem("uat-onboarded"))setTimeout(startOnboarding,800);
}
...
function switchTab(name,silent){
  ...
  const bc=document.getElementById("breadcrumb");if(bc)bc.innerHTML=`<span>${TAB_NAMES[name]||name}</span>`;
...
// ONBOARDING
const OS=[
  {t:".phead",title:"Welcome!",text:"This dashboard gives you a real-time overview of all UAT testing activity."},
  {t:".kpis",title:"Key Metrics",text:"Total runs, pass rate, agent count, and screenshots at a glance."},
  {t:".stab[data-tab=runner]",title:"Test Runner",text:"Select scripts, configure variables, run tests, view progress."},
  {t:".stab[data-tab=agents]",title:"AI Agents",text:"9 specialized pathology agents for all LIS systems."},
  {t:".stab[data-tab=aitools]",title:"AI Tools",text:"Generate scripts from plain English, auto-remediation, fine-tuning."}
];
let oi=0;
function startOnboarding(){if(!localStorage.getItem("uat-onboarded")){localStorage.setItem("uat-onboarded","1");oi=0;showO();}}
function showO(){
  const s=OS[oi];if(!s)return;
  const el=document.querySelector(s.t);if(!el)return;
  toast(`${oi+1}/${OS.length}: ${s.title} — ${s.text}`,"");
  if(s.t.startsWith(".stab"))switchTab(s.t.match(/data-tab=(\w+)/)?.[1]||"dashboard");
  setTimeout(()=>{oi++;if(oi<OS.length)showO();},4000);
}
...
async function onPatientSelect(){
  ...
  d.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:2px;font-size:12px"><span style="color:var(--t2)">NHS:</span><span><strong>${p.nhsNumber}</strong></span><span style="color:var(--t2)">Name:</span><span>${p.title} ${p.forename} ${p.surname}</span><span style="color:var(--t2)">DOB:</span><span>${p.dob} (${p.age})</span><span style="color:var(--t2)">Gender:</span><span>${p.gender}</span></div>`;
...
async function onScriptChange(){
  ...
  document.getElementById("metaSystem").textContent=r.system||"N/A";document.getElementById("metaSteps").textContent=r.steps.length;
...
function buildVariablesForm(v){
  ...
  if(!v.length){c.innerHTML='<div class="empty" style="padding:6px">No variables needed</div>';return;}
  v.forEach(v2=>{const d=document.createElement("div");d.className="variable-field";
  const l=document.createElement("label");l.textContent=v2.label||v2.name;l.htmlFor=`var-${v2.name}`;
  const i=document.createElement("input");i.id=`var-${v2.name}`;i.name=v2.name;i.type=v2.type==="password"?"password":"text";i.placeholder=v2.default||v2.label||v2.name;i.value=v2.default||"";if(v2.required)i.required=true;
...
async function startRun(){
  ...
  resetRunUI();document.getElementById("runBtn").style.display="none";document.getElementById("cancelBtn").style.display="inline-flex";
...
function connectSSE(id){
  ...
  eventSource.addEventListener("complete",e=>{const d=JSON.parse(e.data);lastRunResult=d;addLog("info",`Complete: ${d.passed}/${d.totalSteps}`);onRunComplete(d);});
  eventSource.onerror=()=>{};
}
async function cancelRun(){if(!currentRunId)return;try{await fetch(`/api/runs/${currentRunId}/cancel`,{method:"POST"});toast("Run cancelled","warn");}catch{}}
function resetRunUI(){
  ...
  document.getElementById("screenshotGrid").innerHTML='<div class="empty">Screenshots appear here.</div>';
...
function resetButtons(){document.getElementById("runBtn").style.display="inline-flex";document.getElementById("cancelBtn").style.display="none";}
function renderStepList(){
  const c=document.getElementById("stepList");c.innerHTML="";
  stepsData.forEach(s=>{const d=document.createElement("div");d.className="timeline-item";d.id=`step-${s.id}`;
  ...
  d.appendChild(desc);d.appendChild(m);if(s.error){const e=document.createElement("div");e.className="step-error";e.textContent=s.error;d.appendChild(e);}c.appendChild(d);});
}
function updateStepStatus(id,status,error,elapsed){
  ...
  m.innerHTML=`<span>Step ${stepsData.findIndex(s=>s.id===id)+1}</span>${status==="running"?'<span>Running...</span>':status==="pass"?'<span style="color:var(--sut)">Pass</span>':status==="fail"?'<span style="color:var(--dat)">Failed</span>':''}${elapsed?`<span>${elapsed}</span>`:''}`;
...
function addScreenshot(data){
  screenshotsData.push(data);const g=document.getElementById("screenshotGrid"),e=g.querySelector(".empty");if(e)e.remove();
...
async function uploadToPractitest(){
  ...
  toast(d.success?`Uploaded ${d.results?.filter?.(r=>r.status==="uploaded").length||0}`:`Upload: ${d.message||"Check config"}`,d.success?"success":"error");
...
async function dashPreviewCSV(){const c=await readCSVFile("dashCsvInput");if(!c)return;previewCSV();}
async function previewCSV(){const c=await readCSVFile("csvFileInput");if(!c)return;try{const d=await(await fetch("/api/scripts/upload-csv-test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({csv:c})})).json();if(d.error){toast("CSV: "+d.error,"error");return;}toast(`${d.total} cases previewed`,"success");}catch(err){toast("Preview: "+err.message,"error");}}
async function dashImportCSV(){const c=await readCSVFile("dashCsvInput");if(!c)return;importCSV();}
async function importCSV(){const c=await readCSVFile("csvFileInput");if(!c)return;try{const d=await(await fetch("/api/scripts/upload-csv",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({csv:c})})).json();if(d.error){toast("Import: "+d.error,"error");return;}toast(`Imported ${d.imported} scripts`,"success");await loadScripts();populateLoadScripts();}catch(err){toast("Import: "+err.message,"error");}}

function addLog(l,m){const c=document.getElementById("logContainer"),e=c.querySelector(".empty");if(e)e.remove();const d=document.createElement("div");d.className="log-entry";const t=document.createElement("span");t.className="log-time";t.textContent=new Date().toLocaleTimeString();const msg=document.createElement("span");msg.className=`log-msg ${l}`;msg.textContent=m;d.appendChild(t);d.appendChild(msg);c.appendChild(d);c.scrollTop=c.scrollHeight;}
function clearLog(){document.getElementById("logContainer").innerHTML='<div class="empty">Log cleared.</div>';}

async function checkAIStatus(){try{const d=await(await fetch("/api/ai/status")).json();const sd=document.getElementById("sidebarStatus");if(sd)sd.className=`sdot ${d.enabled?"connected":"disconnected"}`;}catch{}}
...
function selectAgent(agent){
  ...
  <div class="field"><label>URL</label><input id="agentUrl" placeholder="System URL"></div>
...
async function executeAgent(){
  ...
  document.getElementById("agentResult").innerHTML=`<div style="font-size:13px"><strong>Status:</strong> ${d.status} | <strong>Passed:</strong> ${d.passed||0}/${d.total||0} | <strong>Screenshots:</strong> ${d.screenshots||0}</div>`;
...
async function generateScriptNL(){
  ...
  try{const d=await(await fetch("/api/nl/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text,systems})})).json();const el=document.getElementById("nlResult");el.classList.remove("hidden");if(d.error){el.innerHTML=`<div style="color:var(--dat)">${d.error}</div>`;return;}el.innerHTML=`<pre>${JSON.stringify(Array.isArray(d)?d:d.steps||d,null,2)}</pre>`;toast(`Generated ${Array.isArray(d)?d.length:0} steps`,"success");}catch(err){toast("NL: "+err.message,"error");}
}
...
async function loadFHIRCodes(){try{const d=await(await fetch("/api/fhir/codes")).json();const el=document.getElementById("fhirCodes");if(!el)return;el.innerHTML=(d.observations||[]).map(o=>`<div><span><strong>${o.code}</strong>: ${o.display}</span><span style="color:var(--t2)">${o.unit}</span></div>`).join("");}catch{}}
...
async function testFHIRServer(){try{const d=await(await fetch("/api/fhir/r4/metadata")).json();document.getElementById("fhirServerResult").classList.remove("hidden");document.getElementById("fhirServerResult").innerHTML=`<pre>${JSON.stringify({fhirVersion:d.fhirVersion},null,2)}</pre>`;}catch(err){toast("FHIR: "+err.message,"error");}}
...
async function refreshMetrics(){
  ...
  el.innerHTML=`<div class="mkpi"><span class="mkpiv">${d.runsStarted||0}</span><span class="mkpil">Runs</span></div><div class="mkpi"><span class="mkpiv">${d.passRate||0}%</span><span class="mkpil">Rate</span></div><div class="mkpi"><span class="mkpiv">${d.agentCount||0}</span><span class="mkpil">Agents</span></div><div class="mkpi"><span class="mkpiv">${Math.floor((d.uptimeSeconds||0)/86400)}d</span><span class="mkpil">Uptime</span></div><div class="mkpi"><span class="mkpiv">${d.screenshotsCaptured||0}</span><span class="mkpil">Shots</span></div><div class="mkpi"><span class="mkpiv">v${d.version||"?"}</span><span class="mkpil">Version</span></div>`;
  }catch{}
  try{const d=await(await fetch("/api/insights")).json();
  const rr=document.getElementById("recentRuns");if(!rr)return;
  if(!d.recentRuns?.length){rr.innerHTML='<div class="empty">No runs yet.</div>';return;}
  rr.innerHTML=d.recentRuns.map(r=>`<div class="log-entry"><span class="log-time" style="color:${r.status==="pass"?"var(--sut)":r.status==="fail"?"var(--dat)":"var(--t2)"}">●</span><span class="log-msg" style="font-size:12px"><strong>${r.script||r.id?.slice(0,16)}</strong> <span style="color:var(--t2)">${r.system||""}</span></span><span style="font-size:11px;color:var(--t2);flex-shrink:0">${r.passed||0}/${(r.passed||0)+(r.failed||0)}</span></div>`).join("");
  }catch{}
}
function updateHealthList(){
  const el=document.getElementById("healthList");if(!el)return;
  el.innerHTML=`<div class="log-entry"><span class="log-time" style="color:var(--sut)">●</span><span class="log-msg" style="font-size:12px"><strong>API Server</strong> <span style="color:var(--t2)">Online</span></span></div>
  <div class="log-entry"><span class="log-time" style="color:var(--sut)">●</span><span class="log-msg" style="font-size:12px"><strong>Cloudflare Tunnel</strong> <span style="color:var(--t2)">Connected</span></span></div>
  <div class="log-entry"><span class="log-time" id="healthAiDot" style="color:var(--t2)">●</span><span class="log-msg" style="font-size:12px"><strong>AI Backend</strong> <span id="healthAi" style="color:var(--t2)">Checking...</span></span></div>
  <div class="log-entry"><span class="log-time" id="healthPtDot" style="color:var(--t2)">●</span><span class="log-msg" style="font-size:12px"><strong>PractiTest</strong> <span id="healthPt" style="color:var(--t2)">Checking...</span></span></div>`;
...
async function loadSchedule(){
  ...
  if(!j.length){el.innerHTML='<div class="empty">No jobs. <button class="btn btn-text btn-sm" onclick="showAddSchedule()">Add one</button></div>';return;}
  el.innerHTML=j.map(j2=>`<div class="log-entry"><span class="log-time">${j2.enabled?"🟢":"🔴"}</span><span class="log-msg" style="font-size:12px"><strong>${j2.cron}</strong> — ${j2.script||j2.agentId||"?"}</span><span><button class="btn btn-text btn-sm" onclick="deleteSchedule('${j2.id}')" style="color:var(--dat)">✕</button></span></div>`).join("");
  }catch{}
}
...
async function loadAudit(){const d=document.getElementById("auditDate")?.value||new Date().toISOString().slice(0,10);try{const e=await(await fetch(`/api/audit?date=${d}&limit=100`)).json();const el=document.getElementById("auditLog");if(!e.length){el.innerHTML='<div class="empty">No entries.</div>';return;}el.innerHTML=e.map(e2=>`<div class="log-entry"><span class="log-time">${new Date(e2.timestamp).toLocaleTimeString()}</span><span class="log-msg">${e2.action}</span></div>`).join("");toast(`${e.length} entries`,"success");}catch(err){toast("Audit: "+err.message,"error");}}
...
async function loadMobileProfiles(){try{const p=await(await fetch("/api/mobile/profiles")).json();const el=document.getElementById("mobileProfiles");if(!el)return;el.innerHTML=p.map(p2=>`<div style="padding:4px 0;border-bottom:1px solid var(--b);display:flex;justify-content:space-between;font-size:12px"><span><strong>${p2.name}</strong></span><span style="color:var(--t2)">${p2.viewport}</span></div>`).join("");}catch{}}
async function loadOAuthStatus(){try{const d=await(await fetch("/api/oauth/status")).json();const el=document.getElementById("oauthStatus");if(!el)return;el.innerHTML=`<div class="mkpis" style="grid-template-columns:repeat(3,1fr)"><div class="mkpi"><span class="mkpiv">${d.enabled?"🟢":"🔴"}</span><span class="mkpil">Enabled</span></div><div class="mkpi"><span class="mkpiv">${d.clientCount||0}</span><span class="mkpil">Clients</span></div><div class="mkpi"><span class="mkpiv">${d.activeTokens||0}</span><span class="mkpil">Tokens</span></div></div>`;}catch{}}
