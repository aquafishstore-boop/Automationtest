...
const ONBOARDING_STEPS=[
  {target:"#tab-dashboard .dashboard-header",title:"Welcome to UAT Tester",text:"This dashboard gives you an overview of all testing activity. Check stats, recent runs, and system health at a glance."},
  {target:"#tab-dashboard .quick-actions-panel",title:"Quick Actions",text:"Jump to any feature from here: run a test, launch an AI agent, write with AI, or generate FHIR data."},
  {target:".sidebar-tab[data-tab=runner]",title:"Test Runner",text:"Select scripts, configure variables, run tests, and view step-by-step progress with screenshots."},
...
// ===== ONBOARDING =====
function initOnboarding(){if(!localStorage.getItem("uat-onboarded"))setTimeout(()=>startOnboarding(),500);}
...
function showOnboardingStep(){
  ...
  const rect=el.getBoundingClientRect();t.style.left=`${Math.min(rect.left,window.innerWidth-340)}px`;t.style.top=`${rect.bottom+10}px`;
  if(step.target.startsWith(".sidebar")){
    t.style.left="210px";t.style.top="200px";
    switchTab(step.target.match(/data-tab=(\w+)/)?.[1]||"dashboard");
  }
}
function nextOnboarding(){
  onboardingStep++;if(onboardingStep>=ONBOARDING_STEPS.length){skipOnboarding();return;}
  showOnboardingStep();
}
...
async function onPatientSelect(){
  ...
  d.innerHTML=`<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.15rem;font-size:0.75rem;">
...
function buildVariablesForm(v){
  ...
  if(!v.length){c.innerHTML='<div class="empty-state" style="padding:0.5rem">No variables needed</div>';return;}
  v.forEach(v2=>{
...
function connectSSE(id){
  ...
  eventSource.onerror=()=>{document.getElementById("connectionLabel").textContent="Disconnected";};
}
...
async function _previewCSV(csv){
  try{
    const d=await(await fetch("/api/scripts/upload-csv-test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({csv})})).json();
    ...
    if(el){el.classList.remove("hidden");el.innerHTML=`<div style="background:var(--surface-2);border-radius:var(--radius);padding:0.6rem;font-size:0.75rem;"><strong>${d.total} test cases</strong> found</div>`;}
...
async function _importCSV(csv){
  try{
    const d=await(await fetch("/api/scripts/upload-csv",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({csv})})).json();
    if(d.error){toast("Import: "+d.error,"error");return;}
    toast(`Imported ${d.imported} scripts`,"success");
...
async function checkAIStatus(){
  try{
    ...
    const sd=document.getElementById("sidebarStatus");if(sd)sd.className=`sidebar-dot ${d.enabled?"connected":"disconnected"}`;
  }catch{}
}
...
async function loadAgents(){
  try{
    const r=await(await fetch("/api/agents")).json();const g=document.getElementById("agentGrid");if(!g)return;g.innerHTML="";
    const badge=document.getElementById("agentCountBadge");if(badge)badge.textContent=r.length;
    r.forEach(a=>{
      const c=document.createElement("div");c.className="agent-card";c.onclick=()=>selectAgent(a);
...
function selectAgent(agent){
  ...
  document.getElementById("agentRunForm").innerHTML=`<div class="form-group"><label>Workflow</label><select id="agentWorkflow">${agent.testCases.map(t=>`<option>${t}</option>`).join("")}</select></div>
  <div class="form-group"><label>Patient NHS</label><input id="agentNhs" placeholder="e.g. 999 057 5924"></div>
  <div class="form-group"><label>URL (optional)</label><input id="agentUrl" placeholder="System URL"></div>
  <div class="form-group"><label>Username</label><input id="agentUser" placeholder="Login"></div>
  <div class="form-group"><label>Password</label><input id="agentPass" type="password" placeholder="Password"></div>`;
}
...
async function executeAgent(){
  ...
  try{
    ...
    const rd=document.getElementById("agentResult");rd.classList.remove("hidden");rd.innerHTML=`<div style="font-size:0.8rem;"><strong>Status:</strong> ${d.status} | <strong>Passed:</strong> ${d.passed||0}/${d.total||0} | <strong>Screenshots:</strong> ${d.screenshots||0}</div>`;
...
async function generateScriptNL(){
  ...
  try{
    const d=await(await fetch("/api/nl/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text,systems})})).json();
...
async function suggestTests(){
  ...
  try{const d=await(await fetch("/api/nl/suggest",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({system:s})})).json();
  const el=document.getElementById("nlResult");el.classList.remove("hidden");el.innerHTML=`<pre>${d.result||JSON.stringify(d,null,2)}</pre>`;
...
async function loadFineTuneStats(){
  ...
  try{const f=await(await fetch("/api/fine-tune/files")).json();const el=document.getElementById("ftFileList");if(!f.length){el.innerHTML='<div style="color:var(--text-secondary);padding:0.5rem;">No data yet.</div>';return;}el.innerHTML=f.map(f2=>`<div><span>${f2.filename}</span><span style="color:var(--text-secondary)">${f2.examples} ex</span></div>`).join("");}catch{}
}
async function collectTraining(){
  if(!lastRunResult){toast("No completed run","error");return;}
  try{const d=await(await fetch("/api/fine-tune/collect",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({steps:lastRunResult.steps||[],system:lastRunResult.system||"general"})})).json();toast(`Collected ${d.count||0} examples`,"success");loadFineTuneStats();}catch(err){toast("Collect: "+err.message,"error");}
}
async function submitFineTune(){
  try{const d=await(await fetch("/api/fine-tune/submit",{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"})).json();toast(d.submitted?`Submitted (${d.totalExamples} ex)`:`Error: ${d.error||"Unknown"}`,d.submitted?"success":"error");}catch(err){toast("Submit: "+err.message,"error");}
}

// ===== FHIR =====
async function loadFHIRCodes(){
  try{const d=await(await fetch("/api/fhir/codes")).json();const el=document.getElementById("fhirCodes");if(!el)return;el.innerHTML=(d.observations||[]).map(o=>`<div><span><strong>${o.code}</strong>: ${o.display}</span><span style="color:var(--text-secondary)">${o.unit}</span></div>`).join("");}catch{}
}
async function generateFHIR(){
  const c=parseInt(document.getElementById("fhirCount")?.value)||1,g=document.getElementById("fhirGender")?.value||"";
  try{const d=await(await fetch(`/api/fhir/patients?count=${c}${g?"&gender="+g:""}`)).json();const el=document.getElementById("fhirResult");el.classList.remove("hidden");el.innerHTML=`<pre>${JSON.stringify(d,null,2)}</pre>`;toast(`Generated ${d.generated||0} patients`,"success");}catch(err){toast("FHIR: "+err.message,"error");}
}
...
async function refreshMetrics(){
  ...
  try{
    ...
    let html=`<h3 style="font-size:0.8rem;font-weight:600;margin:0.7rem 0 0.35rem;">Run History</h3><table>${(d.recentRuns||[]).slice(0,5).map(r=>`<tr><td style="font-size:0.72rem;">${new Date(r.date).toLocaleTimeString()}</td><td style="font-size:0.72rem;">${r.script||r.id?.slice(0,12)}</td><td style="font-size:0.72rem;">${r.status}</td><td style="font-size:0.72rem;text-align:right;">${r.passed||0}/${(r.passed||0)+(r.failed||0)}</td></tr>`).join("")}</table>`;
    el.innerHTML=html;
    // Recent runs on dashboard
    const rr=document.getElementById("recentRuns");if(!rr)return;
    if(!d.recentRuns?.length){rr.innerHTML='<div class="empty-state">No runs yet. <button class="btn btn-text" onclick="switchTab(\'runner\')">Run your first test →</button></div>';return;}
    rr.innerHTML=d.recentRuns.map(r=>`<div class="run-entry"><span class="run-status ${r.status}"></span><div class="run-name"><span>${r.script||r.id?.slice(0,16)}</span><span class="run-meta-text">${r.system||"Test"}</span></div><span class="run-stat">${r.passed||0}/${(r.passed||0)+(r.failed||0)}</span></div>`).join("");
  }catch{}
}
...
function updateHealthList(){
  ...
  el.innerHTML=`<div class="health-item"><span class="health-dot up"></span><span class="health-name">API Server</span><span class="health-value">Online</span></div>
    <div class="health-item"><span class="health-dot up"></span><span class="health-name">Cloudflare Tunnel</span><span class="health-value">Connected</span></div>
    <div class="health-item"><span class="health-dot up"></span><span class="health-name">AI Backend</span><span class="health-value" id="healthAi">Checking...</span></div>
    <div class="health-item"><span class="health-dot up"></span><span class="health-name">PractiTest</span><span class="health-value" id="healthPt">Checking...</span></div>`;
  fetch("/api/ai/status").then(r=>r.json()).then(d=>{document.getElementById("healthAi").textContent=d.enabled?"Connected":"Offline";}).catch(()=>{document.getElementById("healthAi").textContent="Offline";});
  fetch("/api/practitest/status").then(r=>r.json()).then(d=>{document.getElementById("healthPt").textContent=d.configured?"Connected":"Not configured";}).catch(()=>{});
}

// ===== SCHEDULE =====
async function loadSchedule(){
  try{const j=await(await fetch("/api/schedule")).json();const el=document.getElementById("scheduleList");if(!el)return;
  if(!j.length){el.innerHTML='<div class="empty-state">No scheduled jobs. <button class="btn btn-text" onclick="showAddSchedule()">Add one →</button></div>';return;}
  el.innerHTML=j.map(j2=>`<div style="padding:0.35rem 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;font-size:0.75rem;">
    <div><strong>${j2.cron}</strong> — ${j2.script||j2.agentId||"?"} ${j2.enabled?"🟢":"🔴"}</div>
    <div style="display:flex;gap:0.25rem;">
      <button class="btn btn-text btn-sm" onclick="toggleSchedule('${j2.id}')">${j2.enabled?"Disable":"Enable"}</button>
      <button class="btn btn-text btn-sm" onclick="deleteSchedule('${j2.id}')" style="color:var(--danger)">Delete</button>
    </div></div>`).join("");
  }catch{}
}
...
async function addSchedule(){
  const c=document.getElementById("schedCron")?.value,s=document.getElementById("schedScript")?.value;if(!c||!s){toast("Cron and script required","error");return;}
  try{await(await fetch("/api/schedule",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({cron:c,script:s,agentId:s})})).json();toast("Job scheduled","success");hideScheduleForm();loadSchedule();}catch(err){toast("Schedule: "+err.message,"error");}
}
...
async function loadAudit(){
  ...
  try{const e=await(await fetch(`/api/audit?date=${d}&limit=100`)).json();const el=document.getElementById("auditLog");
  if(!e.length){el.innerHTML='<div class="empty-state">No entries for this date.</div>';return;}
  el.innerHTML=e.map(e2=>`<div class="log-entry"><span class="log-time">${new Date(e2.timestamp).toLocaleTimeString()}</span><span class="log-msg ${e2.severity==="warn"?"warn":e2.severity==="error"?"error":""}">${e2.action} — ${e2.user||"anon"}</span></div>`).join("");
  toast(`${e.length} entries loaded`,"success");
  }catch(err){toast("Audit: "+err.message,"error");}
}

// ===== LOAD TEST =====
async function generateK6(){
  const f=document.getElementById("loadScriptSelect")?.value;if(!f){toast("Select a script","error");return;}
  try{const s=await(await fetch(`/api/scripts/${f}`)).json();const c=await(await fetch("/api/load-test/generate-k6",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({script:s,vus:10})})).text();
  document.getElementById("loadResult").classList.remove("hidden");document.getElementById("loadResult").textContent=c;}catch(err){toast("k6: "+err.message,"error");}
}
async function generateArtillery(){
  const f=document.getElementById("loadScriptSelect")?.value;if(!f){toast("Select a script","error");return;}
  try{const s=await(await fetch(`/api/scripts/${f}`)).json();const d=await(await fetch("/api/load-test/generate-artillery",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({script:s,vus:10})})).json();
  document.getElementById("loadResult").classList.remove("hidden");document.getElementById("loadResult").textContent=JSON.stringify(d,null,2);}catch(err){toast("Artillery: "+err.message,"error");}
}

// ===== MOBILE =====
async function loadMobileProfiles(){
  try{const p=await(await fetch("/api/mobile/profiles")).json();const el=document.getElementById("mobileProfiles");if(!el)return;
  el.innerHTML=p.map(p2=>`<div style="padding:0.3rem 0;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;font-size:0.76rem;"><span><strong>${p2.name}</strong></span><span style="color:var(--text-secondary)">${p2.viewport} ${p2.touch?"📱":"🖥"}</span></div>`).join("");}catch{}
}

// ===== OAUTH =====
async function loadOAuthStatus(){
  try{const d=await(await fetch("/api/oauth/status")).json();const el=document.getElementById("oauthStatus");if(!el)return;
  el.innerHTML=`<div class="mini-stats" style="grid-template-columns:repeat(3,1fr);"><div class="mini-stat"><span class="mini-value">${d.enabled?"🟢":"🔴"}</span><span class="mini-label">Enabled</span></div><div class="mini-stat"><span class="mini-value">${d.clientCount||0}</span><span class="mini-label">Clients</span></div><div class="mini-stat"><span class="mini-value">${d.activeTokens||0}</span><span class="mini-label">Tokens</span></div></div>`;}catch{}
}
