...
import { getBestSnapshot } from "./aom-parser.js";
import { captureAndCompare } from "./visual-regression.js";
import { getActiveFrame, getAllFramesInfo, clickInFrame, fillInFrame, waitForFrame } from "./frame-manager.js";
...
10. screenshot the order details`;
  ...
  async _think(goal, dom) {
    // Use AOM snapshot with fallback to filtered DOM
    const snapshot = await getBestSnapshot(this.page, dom);
    const content = snapshot.content || "(no interactive elements found)";

    const prompt = `You are controlling a browser for pathology system testing.
UI state captured via: ${snapshot.source === "aom" ? "Accessibility Tree (AOM)" : "Filtered DOM"}
${snapshot.actionableCount ? `Actionable elements: ${snapshot.actionableCount}` : ""}
...
Return ONLY valid JSON with: action (click|type|select|wait|assert|navigate|screenshot), selector (CSS selector to interact with), value (text to type if needed), assertion (text to verify if needed), confidence (0-1).`;
  ...
  async _act(action) {
    ...
    try {
      // Resolve active frame (handle iframes and shadow DOM)
      const { frame } = await getActiveFrame(this.page);

      // Visual regression check before action
      const visualCheck = await captureAndCompare(this.page, `${this.currentStepIndex}_${type}`);
      if (visualCheck.regression) {
        this._log("warn", `Visual regression detected at step ${this.currentStepIndex}`);
      }
      ...
      switch (type) {
        ...
        case "click":
          if (selector) {
            await waitForFrame(frame, selector, 8000);
            await clickInFrame(frame, selector);
          } else if (description) {
            const ai = await this._think(`Click: ${description}`, await this.page.content());
            if (ai?.selector) {
              await waitForFrame(frame, ai.selector, 5000);
              await clickInFrame(frame, ai.selector);
        ...
        case "type":
          if (selector && value !== undefined) {
            await waitForFrame(frame, selector, 5000);
            await fillInFrame(frame, selector, value);
          }
          break;
        ...
        case "wait":
          if (selector) {
            await waitForFrame(frame, selector, 15000);
          } else {
        ...
        case "verify":
          if (selector) {
            await waitForFrame(frame, selector, 5000);
            const text = await frame.textContent(selector).catch(() => "");
